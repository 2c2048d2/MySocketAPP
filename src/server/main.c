        ...
        if (data != NULL) {
          ...
          case DATA_PACK_TYPE_COMMAND:
            ...
            case DATA_PACK_TYPE_COMMAND_QUIT:
              ...
              send_data_pack(
                  gen_data_pack(DATA_PACK_TYPE_STATUS, subtype, 0, NULL, NULL),
                  fd, 1);
              running_flag = 0;
              break;
            case DATA_PACK_TYPE_COMMAND_GET_INFO:
              ...
              send_data_pack(gen_data_pack(DATA_PACK_TYPE_INFO, subtype,
                                           strlen(message) + 1, message, NULL),
                             fd, 1);
              break;
            case DATA_PACK_TYPE_COMMAND_MKDIR:
              printf("正在创建文件夹%s\n", data->payload);
              if (mkdir(data->payload, 0777) < 0)
                perror("mkdir");
              break;
            }
            break;
          case DATA_PACK_TYPE_FILE:
            ...
            case DATA_PACK_TYPE_FILE_START:
              printf("收到新文件，文件名:%s\n", data->payload);
              int new_file_fd = open(data->payload,
                                     O_WRONLY | O_TRUNC | O_CREAT, DEFFILEMODE);
              if (new_file_fd == -1) {
                perror("文件打开失败");
                subtype.status_type = DATA_PACK_TYPE_STATUS_FAILED;
                send_data_pack(gen_data_pack(DATA_PACK_TYPE_STATUS, subtype, 0,
                                             NULL, NULL),
                               fd, 1);
              } else {
                file_size[fd] = 0;
            ...
            case DATA_PACK_TYPE_FILE_SENDING:
              ...
              write(file_fd[fd], data->payload, data->data_length);
              break;
